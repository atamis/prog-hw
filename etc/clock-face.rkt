...
(define clock-radius 200)
(define face-color "black")
(define face-style "solid")

(define sec-hand-color "white")
(define sec-hand-length (* clock-radius 9/10))

(define minute-hand-color "white")
(define minute-hand-length (* clock-radius 2/3))

(define hour-hand-color "white")
...
(define (draw-arrow degrees length color)
  (rotate
     (remainder (round (- 180 degrees)) 360) ;; Instead of round, we should use fraction->decimal
     (put-pinhole 0 0
...
(define final-ticks (animate clock))
