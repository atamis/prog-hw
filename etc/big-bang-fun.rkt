...
(define (create-UFO-scene state)
  (scene+line (overlay/align "left" "top" 
              ...
              (ufo-state-locx state) (ufo-state-locy state);; This code draws the acceleration line. scene-line is really annoying in that their first argment is the image to operate on, while place-image has the image as the last argument. This leads to confusing code.
              (+ (ufo-state-locx state) (ufo-state-vecx state)) (+ (ufo-state-locy state) (ufo-state-vecy state))
              "black"))
